    ...
    int maxSprites, int maxAnims, int cwfFlag)
{
    ...
    if (LMC_Init(appName, baseWidth, baseHeight, maxWidth, maxHeight, aspect, fps, maxLayers, maxSprites, maxAnims))
        m_legacyMachine = LMC_GetContext();
    if (!m_legacyMachine)
        return false;
    ...
    JoypadManager* pJoypadManager = JoypadManager::GetJoypadManager();

    // Initialize the joypad manager and load the joypad configuration database.
    if (pJoypadManager->IntializeJoypadDatabase("Joypads.csv"))
    {
        LMC_SetAutoConfigureJoypadCallback(JoypadAutoConfig);
    }
...
{
    if (!LMC_LoadCore(coreFilename, false))
        ErrorQuit("Failed to load core. ", LMC_GetErrorString(LMC_GetLastError()));

    if (strcmp(softwareFilename, "none") == 0)
    {
        if (!LMC_LoadContent(NULL))
            ErrorQuit("Failed to load core without content. ", LMC_GetErrorString(LMC_GetLastError()));
    }
    else
    {
